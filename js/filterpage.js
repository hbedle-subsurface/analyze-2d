/* ============================ filter pages ============================
   Steps 3 to 5 share one layout: the input, the kept result and the removed
...
async function filterPage(cfg){
  ...
  FP.clip = seisClip(input.data, input);
  if (stored){
    FP.kept = stored.data;
  ...
  const run = async () => {
    ...
    await stageSave(cfg.stage, {data: out, nx: input.nx, ns: input.ns, dt: input.dt, j0: input.j0},
                    {i0: input.i0, istep: input.istep, params: FP.params, from: input.stage});
...
function markFlow(done){
  ...
  while (n){ if (n.getAttribute("href") && /[3-5]-/.test(n.getAttribute("href"))) n.classList.add("skipped"); n = n.nextElementSibling; }
}
...
function drawPanels(){
  ...
  drawSeis("pKept", input, kept, FP.clip, FP.on ? "output of this step, same color scale as the input" :
