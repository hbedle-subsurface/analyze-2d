...
because the NLOG SEG-Y was not at hand when the tool was built. That rebuild
kept every second trace of the array, so the file holds half the traces that
`scan-lecture` draws (20 m spacing rather than 10 m). The array itself was
...
This keeps every 4th CDP (10 m) and 0 to 3.0 s, writes IEEE float with the CDP
...
changes. The interim copy can be rebuilt with every trace of the `scan-lecture` array,
at the same 10 m spacing that tool draws, with

    python python/make_sample.py --scan-lecture ../scan-lecture/data

`--step N` keeps every Nth trace instead, for either source.
