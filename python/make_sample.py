...
Two sources are supported.
  ...
  python make_sample.py --segy L2EBN2020ASCAN029_PreSTM_final_full.sgy
      ...
      keeps every 8th CDP (20 m) and 0-3.0 s, and writes IEEE float SEG-Y with the
  ...
  python make_sample.py --scan-lecture ../scan-lecture/data
      ...
      That array is 8-bit, clipped at the 99th percentile of absolute amplitude and
      time-gained, so it stands in only until the NLOG file is used.
...
def from_segy(path, step=8, t_max=3.0):
...
def from_scan_lecture(folder, step=2):
    folder = Path(folder)
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    shape = from_segy(a.segy) if a.segy else from_scan_lecture(a.scan_lecture)
